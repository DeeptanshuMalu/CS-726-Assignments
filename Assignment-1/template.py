...
class Inference:
    ...
    def get_junction_tree(self):
        ...
        self.opt_order = get_message_passing_order(
            self.junction_tree, list(self.junction_tree.keys())[0]
        )
    ...
    def assign_potentials_to_cliques(self):
        ...
        def multiply_potentials(
            potential1, potential2, potn1_depends_on, potn2_depends_on
        ):
            potential_new = [None] * (
                2 ** (len(set(potn1_depends_on).union(potn2_depends_on)))
            )
            for val in itertools.product(
                [0, 1], repeat=len(set(potn1_depends_on).union(potn2_depends_on))
            ):
    ...
    def get_z_value(self):
        ...
        def calc_z(message_dict, potentials):
            ...
            for neigh in message_dict[node]:
                ...
                z = multiply_messages(
                    z, message_dict[neigh][node], node, set(node).intersection(neigh)
                )
        ...
        def message_passing_opt_order(junc_tree, potentials, opt_order):
            ...
            for m_clique, neigh in opt_order:
                ...
                for neigh1 in junc_tree[m_clique]:
                    if neigh1 != neigh:
                        potn = multiply_messages(
                            potn,
                            message_dict[neigh1][m_clique],
                            m_clique,
                            set(m_clique).intersection(neigh1),
                        )
                diff = set(m_clique).difference(neigh)
                summing_nodes = set(m_clique).difference(diff)
                message_dict[m_clique][neigh] = condense_message(
                    potn, m_clique, summing_nodes, diff
                )
            return message_dict

        message_dict = message_passing_opt_order(
            self.junction_tree, self.max_clique_potentials, self.opt_order
        )
        ...
        for i, node in enumerate(self.nodes):
            ...
            for neigh in neighbors:
                potential = multiply_messages(
                    potential,
                    self.message_dict[neigh][tuple(super_clique)],
                    super_clique,
                    set(super_clique).intersection(neigh),
                )
            potential = condense_message(
                potential, super_clique, [node], set(super_clique).difference([node])
            )
    ...
    def compute_top_k(self):
        ...
        def multiply_messages(potential, message, node, mesg_depends_on):
            ...
            for val in itertools.product([0, 1], repeat=len(node)):
                ...
                intersection = set_potential_assignment.intersection(
                    set_message_assignment
                )
                ...
                for pot_assignment in potential_assignment:
                    for mesg_assignment in message_assignment:
                        ...
                        if intersection_matches:
                            diff_dict = {
                                var: mesg_assignment[1][var]
                                for var in set_message_assignment.difference(
                                    intersection
                                )
                            }
        ...
        def message_passing_opt_order_top_k(junc_tree, potentials, opt_order):
            ...
            for m_clique, neigh in opt_order:
                ...
                for neigh1 in junc_tree[m_clique]:
                    if neigh1 != neigh:
                        potn_message = multiply_messages(
                            potn_message,
                            message_dict[neigh1][m_clique],
                            m_clique,
                            set(m_clique).intersection(neigh1),
                ...
                summing_nodes = set(m_clique).difference(diff)
                message_dict[m_clique][neigh] = condense_message(
                    potn_message, m_clique, summing_nodes, diff
                )
            return message_dict

        message_dict = message_passing_opt_order_top_k(
            self.junction_tree, self.max_clique_potentials, self.opt_order
        )
        ...
        for neigh in self.junction_tree[sample_node]:
            sample_node_potential = multiply_messages(
                sample_node_potential,
                message_dict[neigh][sample_node],
                sample_node,
                set(sample_node).intersection(neigh),
            )

        top_k_assignments = condense_message(
            sample_node_potential, sample_node, [], sample_node
        )
        ...
        for assgn in top_k_assignments[0]:
            ...
            val = assgn[0] / self.z
            top_k_dict_list.append({"assignment": assign_list, "probability": val})
